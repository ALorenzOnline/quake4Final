		...
		bool tookWeapon = false;
 		for( pos = value; pos != NULL; pos = end ) {
			...
			if ( i == -1 ) {
 			...
 			if ( !gameLocal.world->spawnArgs.GetBool( "no_Weapons" ) || ( weaponName == "weapon_blaster" ) ) {//switched weapon_fists to blaster
...
bool idPlayer::CanBuyItem( const char* itemName )
{
	...
	if(we1==1 && itemName=="weapon_shotgun")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	
	if(we2==1 && itemName=="weapon_machinegun")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(( we3==1 || we3==4) && itemName=="weapon_hyperblaster")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(we4==1 && itemName=="weapon_grenadelauncher")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(we5==1 && itemName=="weapon_rocketlauncher")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(we6==1 && itemName=="weapon_railgun")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(we7==1 && itemName=="weapon_lightning")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(we8==1 && itemName=="weapon_napalmgun")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(a1==1 && itemName=="item_armor_small")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(a2==1 && itemName=="item_armor_large")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(a3==1 && itemName=="ammorefill")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(r1==1 && itemName=="ammo_regen")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(r2==1 && itemName=="health_regen")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
	if(r3==1 && itemName=="damage_boost")
	{
		return( buyStatus == IBS_NOT_ALLOWED );
	}
		...
		if ( zoom ) {
			...
			if(currentWeapon == 4){
				for(int i=0;i<gameLocal.num_entities;i++){
					idEntity *checkEnt = gameLocal.entities[i];
					//checks the if the ent is not null and then checks class type, akes sure its projectile
					if(checkEnt && checkEnt->IsType(idProjectile::GetClassType())){
						for(int j=0;j<gameLocal.num_entities;j++){//if the player is the owner of the grenade explode it
							idEntity *checkPlayer = gameLocal.entities[j];
							if( entityNumber == static_cast<idProjectile*>(checkEnt)->GetOwner()->entityNumber){//checks the owner of the grenade
								static_cast<idProjectile*>(checkEnt)->Explode(NULL,true);
								gameLocal.Printf("my team num");
								gameLocal.Printf("%d",team_num);
							}
						}
					}
				}
				
				gameLocal.Printf("ride the rocket");
			}
			if(currentWeapon == 6){
				gameLocal.Printf("rocket zoom");
				for(int i=0;i<gameLocal.num_entities;i++){
					idEntity *checkRocket = gameLocal.entities[i];
					//checks the if the ent is not null and then checks class type, akes sure its projectile
					if(checkRocket && checkRocket->IsType(idProjectile::GetClassType())){
						
						for(int j=0;j<gameLocal.num_entities;j++){//if the player is the owner of the grenade explode it
							idEntity *checkPlayer = gameLocal.entities[j];
							if( entityNumber == static_cast<idProjectile*>(checkRocket)->GetOwner()->entityNumber){//checks the owner of the grenade
								SetOrigin(static_cast<idProjectile*>(checkRocket)->GetLocalCoordinates(checkRocket->GetPhysics()->GetOrigin()));
							}
						}
					}
				}
...
// MCG - added damage over time
	if ( !inDamageEvent ) {
		gameLocal.Printf("Dot");
		if ( damageDef->dict.GetFloat( "dot_duration" ) ) {
			gameLocal.Printf("dot damaged sent");
			int endTime;
			...
			if ( endTime == -1 || gameLocal.GetTime() + interval <= endTime ) {//post it again
				gameLocal.Printf("dot damage sent 2");
