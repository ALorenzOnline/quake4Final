...
void idMultiplayerGame::SpawnPlayer( int clientNum ) {
	...
	if ( !p->IsFakeClient() && p->team_num == 1 ) {
		...
		if ( !gameLocal.isClient ) {
			...
			//}
		//aal stop right there criminal scum
		if(p->team_num == 1){				
...
//aal will know what players are alive
void idMultiplayerGame::getDeadPlayers(){
		for(int i = 0;i<gameLocal.num_entities;i++)
		{
			...
			{
				gameLocal.activePlayers= gameLocal.activePlayers+1;
				gameLocal.Printf("active Players = ");
				gameLocal.Printf("%d",gameLocal.activePlayers);
			}
		}
				return;
	}
...
void idMultiplayerGame::CheckRespawns( idPlayer *spectator ) {
	//aal checking alive players when checking for respawns
	//idMultiplayerGame *checktheDead;
	//checktheDead->getDeadPlayers();
	for( int i = 0 ; i < gameLocal.numClients ; i++ ) {
		idEntity *ent = gameLocal.entities[ i ];
		...
		if(i  == gameLocal.enemiesCanSpawn)
		{
			p->canSpawn = 1;
		}
