...
void idGameLocal::ServerWriteInitialReliableMessages( int clientNum ) {
	...
	for ( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		//aal write if player was spawned
		networkSystem->ServerSendReliableMessage( clientNum, outMsg, true );
		//aal runs when a player besides the host is spawned
		//gameLocal.allowSpawn[i]=clientNum;
		gameLocal.Printf("%d",gameLocal.allowSpawn[i]);
		idPlayer *randomWeapon;
...
void idGameLocal::RepeaterWriteInitialReliableMessages( int clientNum ) {
	...
	for ( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		gameLocal.Printf("player client Num 2");
	}
...
void idGameLocal::ReadNetworkInfo( int gameTime, idFile* file, int clientNum ) {
	...
	for ( i = 0; i < num; i++ ) {
		...
		numClients = icl + 1;
		gameLocal.Printf("player client Num 3");
	}
