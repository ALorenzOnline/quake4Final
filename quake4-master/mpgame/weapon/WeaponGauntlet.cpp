...
void rvWeaponGauntlet::Spawn ( void ) {
	...
	range			= spawnArgs.GetFloat ( "range", "1000" );//aal Range set Higher was 32, Falcon Punch?
...
// RAVEN END
	...
	if ( tr.fraction >= 1.0f ) {
		//gameLocal.Printf("Its A Hit!");//aal If the tracer hits something print this
		//owner->SetOrigin(tr.endpos);//Hopefully teleports player Lightning gun instead?
		if ( impactEffect ) {
			//gameLocal.Printf("Its a Hit");//aal testing the hitscan
			impactEffect->Stop ( );
			impactEffect = NULL;
