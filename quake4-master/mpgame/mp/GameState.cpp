...
// squirrel: Buying & Deadzone
			...
			if ( gameLocal.mpGame.IsBuyingAllowedInTheCurrentGameMode() )
			{
				...
				//aal start of round set the player team number?
				...
				for( int i = 0; i < gameLocal.numClients; i++ ) {
					...
					if( p == NULL || !p->IsType( idPlayer::GetClassType() ) )
						continue;
					//aal random buy menu assignment when a player joins the game
					p->we1 = gameLocal.random.RandomInt(2);//shotgun
					p->we2 = gameLocal.random.RandomInt(3);//machinegun
					p->we3 = gameLocal.random.RandomInt(3);//hyperblaster
					p->we4 = gameLocal.random.RandomInt(4);//grenade Luancher
					p->we5 = gameLocal.random.RandomInt(2);//rocket launcher
					p->we6 = gameLocal.random.RandomInt(1);//rail gun is harmelss, you can always buy it
					p->we7 = gameLocal.random.RandomInt(4);//zeus gunn
					p->we8 = gameLocal.random.RandomInt(2);//heaphaestus gun
					p->a1 = gameLocal.random.RandomInt(2);//light armor
					p->a2 = gameLocal.random.RandomInt(4);//heavy armor
					p->a3 = gameLocal.random.RandomInt(3);//armor refills
					p->r1 = gameLocal.random.RandomInt(2);//ammo refill
					p->r2 = gameLocal.random.RandomInt(4);//health regen
...
// RITUAL END
			...
			for( i = 0; i < gameLocal.numClients; i++ ) {
				...
				//aal if the player is the first one to spawn they will spawn
				
				if(i==0)
				{
					p->canSpawn=1;//can spawn = 1 player can spawn	
				}
				if(i>0){
					gameLocal.allowSpawn[i] = p;//store the enemy in an array this will be used for wave control
				}
				if(p->team_num == 1  && i == gameLocal.enemiesCanSpawn ){
					p->canSpawn=1; //if the game allows the player to spawn, spawn them 
				}
				...
				//aal if you are a bad guy you have to wait until called for duty
				if( gameLocal.gameType != GAME_TOURNEY && p->canSpawn == 1 ) {
					p->JoinInstance( 0 );
