...
void rvStatManager::Kill( const idPlayer* victim, const idEntity* killer, int methodOfDeath ) {
	...
	if( killer && killer->IsType( idPlayer::GetClassType() ) ) {
		...
		if(killer->team_num==0){
			...
			gameLocal.Printf("%d",gameLocal.enemiesCanSpawn);
			for(int i = 0; i <sizeof(gameLocal.allowSpawn); i++){
...
void rvStatManager::WeaponFired( const idPlayer* player, int weapon, int num ) {
	...
	}else if ( player->GetWeaponIndex( "weapon_machinegun" ) == weapon ) {
		// apparently it processes hits before it does the fire....
		gameLocal.Printf("bang");
	}
...
//aal possibly used to negate team damage? maybe useful for my new game type
